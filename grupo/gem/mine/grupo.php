<?php if(!defined('s7V9pz')) {die();}?>var grversion=2.4;
var alertitle;
var gruploader;
...
function loadgroup($id, e, r) {
    ...
    if ($id != $('.swr-grupo .panel').attr('no') || r == 1 || e.attr('msgload') != undefined) {
        ...
        s = s+'$(".swr-grupo .groupnav > .left > span > span").html(htmlDecode(data[0].pntitle)+"<span>"+data[0].pnsub+"</span>'+typing+'");';
...
function shwrdmre(str, len, nl) {
    ...
    if (maxLength !== "") {
        if ($.trim(str).length > maxLength) {
            ...
            if (len == 0) {
                $shrt = $shrt.replace(/(?:\r\n|\r|\n)/g, '<br>');
                $shrt = '<i class="shortmsg">'+emojione.shortnameToImage(asciiemoji($shrt))+'</i><i class="readmore">'+$(".gphrases > .readmore").text()+'</i>';
                $shrt = $shrt+'<i class="moretext">' + emojione.shortnameToImage(asciiemoji(url2link(str, nl))) + '</i>';
            }
        } else {
            $shrt = emojione.shortnameToImage(asciiemoji(url2link(str, nl)));
        }
    } else {
        $shrt = emojione.shortnameToImage(asciiemoji(url2link(str, nl)));
    }

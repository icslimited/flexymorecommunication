...
function gr_list($do) {
    ...
    if ($do["type"] === "pm") {
        if (isset($GLOBALS["roles"]['privatemsg'][2])) {
            ...
            $src = '"'.$uid.'-%"';
            $srck = '"%-'.$uid.'"';
            $r = db('Grupo', 'q', 'SELECT max(id) as id,gid FROM gr_msgs WHERE companyId = '.$_SESSION['vouser']['companyId'].' AND gid LIKE '.$src.' OR gid LIKE '.$srck.' AND cat="user" GROUP by gid ORDER by id DESC  LIMIT '.$lmt.' OFFSET '.$ofs);
            foreach ($r as $v) {
        ...
        if (!empty($GLOBALS["default"]['pingroup'])) {
            $ar = db('Grupo', 's,v1', 'options', 'type,v2,v1<>', 'gruser', $uid, $GLOBALS["default"]['pingroup']);
        } else {
        ...
        if (!empty($GLOBALS["default"]['pingroup'])) {
            $r = db('Grupo', 'q', 'SELECT * FROM gr_options WHERE id='.$GLOBALS["default"]['pingroup'].' AND companyId = '.$_SESSION['vouser']['companyId'].' AND type="group" || type="group" AND id IN ('.$ar.') ORDER BY id='.$GLOBALS["default"]['pingroup'].' DESC, tms DESC LIMIT '.$lmt.' OFFSET '.$ofs);
        } else {
            $r = db('Grupo', 'q', 'SELECT * FROM gr_options WHERE type="group" AND companyId = '.$_SESSION['vouser']['companyId'].' AND id IN ('.$ar.') ORDER by tms DESC LIMIT '.$lmt.' OFFSET '.$ofs);
        }
        $lk = $lmt-count($r);
        foreach ($r as $v) {
            ...
            } else if ($cu['role'] == 3 && !isset($GLOBALS["roles"]['groups'][7])) {
        ...
        if ($lk != 0) {
            ...
            if (isset($GLOBALS["roles"]['groups'][6]) && isset($GLOBALS["roles"]['groups'][11])) {
                if (!empty($GLOBALS["default"]['pingroup'])) {
                    $rs = db('Grupo', 'q', 'SELECT * FROM gr_options WHERE id<>'.$GLOBALS["default"]['pingroup'].' AND companyId = '.$_SESSION['vouser']['companyId'].' AND type="group" AND id NOT IN ('.$ar.') ORDER by tms DESC LIMIT '.$lk.' OFFSET '.$sofs);
                } else {
                    $rs = db('Grupo', 'q', 'SELECT * FROM gr_options WHERE type="group" AND companyId = '.$_SESSION['vouser']['companyId'].' AND id NOT IN ('.$ar.') ORDER by tms DESC LIMIT '.$lk.' OFFSET '.$sofs);
                }
            } else if (isset($GLOBALS["roles"]['groups'][11])) {
                if (!empty($GLOBALS["default"]['pingroup'])) {
                    $rs = db('Grupo', 'q', "SELECT * FROM gr_options WHERE id<>".$GLOBALS["default"]['pingroup']." AND type='group' AND v3='secret' AND companyId = ".$_SESSION['vouser']['companyId']." AND id NOT IN (".$ar.") ORDER by tms DESC LIMIT ".$lk." OFFSET ".$sofs);
                } else {
                    $rs = db('Grupo', 'q', "SELECT * FROM gr_options WHERE type='group' AND v3='secret' AND companyId = ".$_SESSION['vouser']['companyId']." AND id NOT IN (".$ar.") ORDER by tms DESC LIMIT ".$lk." OFFSET ".$sofs);
                }
            } else if (isset($GLOBALS["roles"]['groups'][6])) {
                if (!empty($GLOBALS["default"]['pingroup'])) {
                    $rs = db('Grupo', 'q', "SELECT * FROM gr_options WHERE id<>".$GLOBALS["default"]['pingroup']." AND type='group' AND v3!='secret' AND companyId = ".$_SESSION['vouser']['companyId']." AND id NOT IN (".$ar.") ORDER by tms DESC LIMIT ".$lk." OFFSET ".$sofs);
                } else {
                    $rs = db('Grupo', 'q', "SELECT * FROM gr_options WHERE type='group' AND companyId = ".$_SESSION['vouser']['companyId']." AND v3!='secret' AND id NOT IN (".$ar.") ORDER by tms DESC LIMIT ".$lk." OFFSET ".$sofs);
                }
        ...
        $list[$i]->loves = gr_shnum(db('Grupo', 's,count(id)', 'msgs', 'type,xtra', 'like', $do['id'])[0][0]);
        ...
        if (isset($do['search']) && strlen($do['search']) > 2 && $ofs == 0) {
            ...
            if (empty($rstb)) {
                ...
                $rstb = db('Grupo', 'q', 'SELECT * FROM gr_options WHERE type="group" AND companyId = '.$_SESSION['vouser']['companyId'].' AND v1 LIKE "%'.$do['search'].'%" AND id IN ('.$ar.') LIMIT 2');
            }
            if (isset($GLOBALS["roles"]['languages'][4])) {

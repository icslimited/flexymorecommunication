            ...
            if (!empty($arg[0]['gid']) && isset($arg[0]['lastid']) && $arg[0]['chat'] == 'on') {
                if (gr_role('access', 'features', '8')) {
                    if (empty($typ) || strtotime('now') > strtotime('+10 seconds', $typ)) {
                        db('Grupo', 'q', 'DELETE FROM gr_logs WHERE type = "typing" AND tms < (CONVERT_TZ(NOW(),@@session.time_zone,"+05:30") - INTERVAL 10 SECOND)');
                        $tmpido = $arg[0]['gid'];
                        if ($arg[0]["ldt"] == 'user') {
                            $tmpido = $arg[0]['gid'].'-'.$uid;
                            if ($arg[0]['gid'] > $uid) {
                        ...
                        $typz = db('Grupo', 's,v3', 'logs', 'type,v1,v2<>', 'typing', $tmpido, $uid, 'ORDER BY id DESC LIMIT 3');
                        $list = array();
                        $list[0] = new stdClass();

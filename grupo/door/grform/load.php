...
function gr_form($do) {
    ...
    if ($do["type"] == "creategroup") {
        ...
        $fields->unleavable = array($lphr['unleavable'], 'select', '0', '-----', 'unleavable', $lphr['yes'], '0', $lphr['no']);
        ...
        if (gr_role('access', 'groups', '2') && $adm == 1 || gr_role('access', 'groups', '7')) {
            ...
            if ($cr && count($cr) > 0) {
                ...
                $unlvb[0] = $lphr['no'];
                $unlvb['unleavable'] = $lphr['yes'];
                $fields->unleavable = array($lphr['unleavable'], 'select', $unlvb, $cr['0']['v6']);
        ...
        foreach ($sys as $s) {
            if ($s['v1'] != 'alert' && $s['v1'] != 'default_font') {
                ...
                if ($s['v1'] === 'userreg' || $s['v1'] === 'non_latin_usernames' || $s['v1'] === 'send_btn_visible' || $s['v1'] === 'recaptcha' || $s['v1'] === 'force_https' || $s['v1'] === 'update_list_periodically' || $s['v1'] === 'unsplash_enable' || $s['v1'] === 'tenor_enable' || $s['v1'] === 'boxed' || $s['v1'] === 'smtp_authentication' || $s['v1'] === 'guest_login' || $s['v1'] === 'email_verification') {

...
function gr_cronjob() {
    ...
    gr_autoroleopts();
    gr_pendmail();
    $logs = db('Grupo', 'q', 'SELECT * FROM gr_logs WHERE type = "browsing" AND tms < (CONVERT_TZ(NOW(),@@session.time_zone,"+05:30") - INTERVAL 120 MINUTE)');
    if (count($logs) > 50) {
        db('Grupo', 'q', 'DELETE FROM gr_logs WHERE type = "browsing" AND tms < (CONVERT_TZ(NOW(),@@session.time_zone,"+05:30") - INTERVAL 120 MINUTE)');
...
function gr_group() {
    ...
    if ($arg[0] === 'valid') {
        ...
        if ($cr[0] && !empty(trim($arg[1]["msg"])) || $cr[0] && $arg[1]["msg"] == 0) {
            ...
            if ($cu[0] && $cu['role'] != 3) {
                ...
                if ($typ != 'system' && !empty($GLOBALS["default"]['sending_limit'])) {
                    $msfld = db('Grupo', 'q', 'SELECT * FROM gr_msgs WHERE uid = '.$uid.' AND type <> "system" AND tms > (CONVERT_TZ(NOW(),@@session.time_zone,"+05:30") - INTERVAL 60 SECOND)');
                    if (count($msfld) >= $GLOBALS["default"]['sending_limit']) {
                        $list[0] = new stdClass();
                        $list[0]->gid = $arg[1]["id"];
                        $list[0]->messageflood = 1;
                        $diff = strtotime(dt()) - strtotime($msfld[count($msfld)-1]['tms']);
                        $diff = 60-$diff;
                        $list[0]->floodwait = $diff.' '.gr_lang('get', 'seconds');
        ...
        $cu = gr_group('user', $orgid, $uid, $arg[1]["ldt"]);
        ...
        if ($cu[0] && $cu['role'] != 3 || isset($rchk['groups']['7'])) {
            if (isset($arg[1]["from"]) && !empty($arg[1]["from"])) {
                ...
                if ($brw > 0) {
                    db('Grupo', 'u', 'logs', 'v3,xtra,tms', 'type,v1', $arg[1]["id"], $arg[1]["ldt"], dt(), 'browsing', $uid);
                } else {
                    db('Grupo', 'i', 'logs', 'type,v1,v3,xtra,tms', 'browsing', $uid, $arg[1]["id"], $arg[1]["ldt"], dt());
                }
            }
            $r = array_reverse($r);
